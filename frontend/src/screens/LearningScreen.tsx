                  ...
                  return {
                    ...
                    lessons: moduleLessons.map((l: any, idx: number) => {
                      ...
                      const dbAssessments = relevantAssessments.filter((asmnt: any) => {
                        ...
                        if (parts[2] && (parts[2] === l.id || resolver.resolveLessonId(parts[2]) === l.id)) return true;
                             ...
                             {lesson.projects && lesson.projects.map((proj: any) => (
                               <div 
                                 ...
                                 onClick={() => {
                                  ...
                                  className={cn(
                                    "p-3 rounded-xl bg-white border shadow-sm transition-all flex items-center justify-between gap-3 cursor-pointer group/item active:scale-[0.99]",
                                    proj.completed
                                      ? "border-emerald-200 hover:shadow-md hover:border-emerald-300"
                                      : proj.isAutoSubmitted
                                        ? "border-amber-200 hover:shadow-md hover:border-amber-300 bg-amber-50/20"
                                        : "border-slate-200 hover:shadow-md hover:border-emerald-300"
                                  )}
                                >
                                  <div className="flex items-start gap-3">
                                    <div className={cn(
                                      "w-9 h-9 rounded-xl flex items-center justify-center shrink-0 shadow-sm text-white transition-transform group-hover/item:scale-105",
                                      proj.isAutoSubmitted && !proj.completed
                                    ...
                                    <div>
                                      ...
                                      <span className="text-[10px] font-semibold text-slate-500 flex items-center gap-1 mt-1">
                                        <Clock className="w-3 h-3" /> {proj.completed ? 'Submitted & Saved' : (proj.isAutoSubmitted ? 'Auto-Submitted (Due Date Passed)' : 'Submission Required')}
                                      </span>
                                    </div>
                                  </div>
                                  {proj.completed ? (
                                    <div className="flex items-center gap-2 shrink-0">
                                      <span className="px-3 py-1.5 rounded-xl font-extrabold text-xs flex items-center gap-1 border shadow-xs bg-emerald-50 text-emerald-700 border-emerald-200">
                                        <CheckCircle2 className="w-4 h-4 text-emerald-600" /> Done
                                      </span>
                                      <button 
                                        type="button"
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                      <span className="px-3 py-1.5 rounded-xl font-extrabold text-xs flex items-center gap-1 border shadow-xs bg-amber-50 text-amber-700 border-amber-200">
                                        <CheckCircle2 className="w-4 h-4 text-amber-600" /> Auto-Submitted
                                      </span>
                                      <button 
                                        type="button"
                                        className="px-2.5 py-1.5 rounded-xl font-extrabold text-xs flex items-center gap-1 text-slate-600 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 transition-all pointer-events-none"
