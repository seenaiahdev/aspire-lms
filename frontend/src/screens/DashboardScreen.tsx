...
export function DashboardScreen() {
  ...
  const [dbLiveSessions, setDbLiveSessions] = useState<any[]>([]);
  const [dbDailySchedules, setDbDailySchedules] = useState<any[]>([]);
  ...
  const selectedDateStr = useMemo(() => {
    ...
    const dd = String(selectedDateNum).padStart(2, '0');
    return `${yyyy}-${mm}-${dd}`;
  }, [selectedYear, selectedMonthIndex, selectedDateNum]);

  // Fetch live sessions and daily schedules on mount / date change
  useEffect(() => {
    async function loadDashboardData() {
      const batchCode = currentUser.batchCode || 'A26W1';
  ...
  const currentTopics = useMemo(() => {
    return dbDailySchedules.map((item, idx) => ({
      id: idx + 9000,
      slug: item.id || `topic-${selectedDateNum}-${idx}`,
      title: item.title || item.subtopic || 'Concept Study',
      category: item.topic || 'General',
      duration: item.time || '45 mins',
      description: item.description || 'Syllabus concept lesson.',
      fullExplanation: item.description || 'Syllabus concept lesson.',
      codeSnippet: `# Practice Challenge\ndef practice():\n    print("${item.title}")\n\npractice()`,
      keyObjectives: item.subtopic ? [item.subtopic] : ['Understand core syntax', 'Review code examples'],
      status: (item.status === 'completed' || item.status === 'in_progress' || item.status === 'upcoming') 
        ? item.status 
        : 'upcoming'
    } as PythonTopic));
    ...
    if (isPastDate) {
      const pastSessions = filtered.map(cls => {
        ...
        return {
          ...
          instructor: { name: cls.instructor || 'Lead Trainer' },
          time: cls.time,
  ...
  const findTopicBySlug = useCallback((slug: string): PythonTopic | null => {
    if (!slug) return null;
    // Resolve deep-linked topics from the REAL daily topics (from live_sessions), not mock data.
    return currentTopics.find((t) => t.slug === slug) || null;
  }, [currentTopics]);
          ...
          <div id="tour-stat-card-1" className="p-5 rounded-3xl bg-white border border-slate-200/80 shadow-2xs hover:shadow-xs transition-all space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-base font-extrabold text-slate-900">{currentUser.progress || 0}% Completed</p>
              </div>
            </div>
            <div className="w-full h-2.5 rounded-full bg-slate-100 overflow-hidden">
              <div className="h-full bg-[#7c3aed] rounded-full transition-all duration-500" style={{ width: `${currentUser.progress || 0}%` }} />
            </div>
          </div>
