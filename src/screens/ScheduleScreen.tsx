...
import { CalendarDays, Clock, MapPin, ChevronLeft, ChevronRight, Radio, FileText, Award, PartyPopper, PlusCircle, Calendar as CalendarIcon } from 'lucide-react';
...
const typeConfig: Record<string, { color: string; icon: any; label: string }> = {
  ...
  event: { color: 'blue', icon: PartyPopper, label: 'Event' },
          ...
          {showAddTask && (
            <Card className="rounded-[2rem] border border-slate-200/70 shadow-sm overflow-hidden bg-white">
              <CardBody className="p-5">
                ...
                <p className="mt-1 text-sm text-slate-500">Create a quick study plan or reminder for the selected date.</p>
                <form onSubmit={handleAddTask} className="mt-5 grid gap-4 sm:grid-cols-2">
                  <div className="sm:col-span-2 space-y-2">
                    <label className="text-sm font-semibold text-slate-900">Task title</label>
                    <input
                      value={taskForm.title}
                      onChange={(e) => handleTaskFormChange('title', e.target.value)}
                      className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none focus:border-[#3b52a4] focus:ring-2 focus:ring-[#3b52a4]/15"
                  ...
                  <div className="space-y-2 sm:col-span-2">
                    <label className="text-sm font-semibold text-slate-900">Course / Notes</label>
                    <input
                  ...
                  <button
                    type="submit"
                    className="sm:col-span-2 inline-flex items-center justify-center rounded-2xl bg-[#1d4ed8] px-5 py-3 text-sm font-semibold text-white hover:bg-[#1e40af] transition-colors"
                  >
                    Save task
                  </button>
                </form>
